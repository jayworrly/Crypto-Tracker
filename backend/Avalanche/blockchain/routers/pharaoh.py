...
def log_pharaoh_transaction(tx, router_info, function_name, params, w3, avax_to_usd, token_loader):
    ...
    if simplified_function_name == "swapCompact":
        log_pharaoh_swap_compact(params, token_loader, w3, tx)
    elif simplified_function_name == "swapMultiCompact":
        log_pharaoh_swap_multi_compact(params, token_loader, w3, tx)
    else:
...
def log_pharaoh_swap_multi_compact(params, token_loader, w3, tx):
    logging.debug(f"Swap Multi Compact params: {params}")
    ...
    # Log multi-swap specific parameters
    if 'amountsIn' in params:
        logging.info("Input Amounts:")
        for i, amount in enumerate(params['amountsIn']):
            token = token_loader.get_token_info(params['path'][i])['label']
            logging.info(f"  {Web3.from_wei(amount, 'ether'):.6f} {token}")
    
    if 'amountOutMin' in params:
        output_token = token_loader.get_token_info(params['tokenY'])['label']
        logging.info(f"Minimum Output Amount: {Web3.from_wei(params['amountOutMin'], 'ether'):.6f} {output_token}")
